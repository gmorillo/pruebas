...
<html>
  ...
  <body>
    ...
    <h2>Laravel Dependent Dropdown  Tutorial With Example</h2>
            ...
            <div class="form-group d-none" id="zona">
                <label for="state">Select State:</label>
                <select name="state" class="form-control " disabled style="width:250px">
                <option>--State--</option>
                </select>
            </div>
      </div> 
               ...
               if(countryID)
               {
                  jQuery.ajax({
                     ...
                     success:function(data)
                     {
                        ...
                        if(countryID != null){
                            jQuery('#zona').removeClass('d-none');
                        }else{
                            jQuery('#zona').addClass('d-none');
                        }

                        if(data != ''){
                            toastr.info('Se tienen datos relacionados')
                        }else{
                            toastr.error('No existen datos relaionados')
                        }
